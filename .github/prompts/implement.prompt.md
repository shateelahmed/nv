...
1. Locate the feature folder under `specs/`. Read `spec.md`, `plan.md`, and
   `tasks.md`. If `tasks.md` is missing, stop and ask me to run `/tasks` first.
2. Work through the unchecked tasks **in order** (or the single task I named).
...
- Follow the existing module layout and Rust conventions in
...
(`cargo` must run outside a restrictive sandbox.) End with a summary of what
changed and the state of `tasks.md`.
